...
Interceptor: {"": "Object;",
  ...
  "%": "CanvasGradient|CanvasPattern|CanvasRenderingContext|CanvasRenderingContext2D|DOMError|DeviceAcceleration|FileError|MediaError|MediaKeyError|Navigator|NavigatorUserMediaError|PositionError|SQLError|SVGAnimatedLength|SVGAnimatedLengthList|SVGAnimatedNumber|SVGAnimatedNumberList|SVGAnimatedString|SVGPointList"
},
...
InputElement: {"": "HtmlElement;height},width}", $isElement: true, $isEventTarget: true, "%": "HTMLInputElement"},
...
["geometrywarsclone", "grid.dart", , K, {
Grid: {"": "Object;points,springList",
  ApplyExplosiveForce$3: function(force, position, radius) {
    var t1, t2, t3, mass, dist2, t4, v;
    for (t1 = this.points, t1 = new H.ListIterator(t1, t1.length, 0, null), t2 = radius * radius; t1.moveNext$0();)
      for (t3 = J.get$iterator$ax(t1._current); t3.moveNext$0();) {
        mass = t3.get$current();
        dist2 = position.$sub(position, mass.get$Position()).get$length2();
        if (dist2 < t2) {
          t4 = mass.Position;
          v = t4.$sub(t4, position);
          v = v.$mul(v, 100);
          v = v.$mul(v, force);
          v = v.$div(v, 10000 + dist2);
          v = v.$mul(v, mass.InverseMass);
          dist2 = mass.acceleration;
          mass.acceleration = dist2.$add(dist2, v);
          mass.damping = mass.damping * 0.6;
        }
      }
  },
  Update$0: function() {
    var t1, t2;
    for (t1 = this.springList, t1 = new H.ListIterator(t1, t1.length, 0, null); t1.moveNext$0();)
      t1._current.Update$0();
  ...
  Draw$1: function(ctx) {
    ...
    for (t1 = this.points, t1 = new H.ListIterator(t1, t1.length, 0, null); t1.moveNext$0();)
      for (t2 = J.get$iterator$ax(t1._current); t2.moveNext$0();) {
        mass = t2.get$current();
        ctx.beginPath();
        ctx.arc(mass.get$Position().storage[0], mass.Position.storage[1], 3, 0, 6.283185307179586, false);
        ctx.fill();
      }
    ctx.restore();
  },
  Grid$2: function(size, spacing) {
    var t1, t2, t3, numColumns, t4, numRows, fixedPoints, y, x, row, t5, t6, x0, column, t7, t8, t9, t10, t11;
    this.springList = P.List_List(null, null);
    t1 = size.width;
    t2 = spacing.storage;
    t3 = t2[0];
    if (typeof t1 !== "number")
    ...
    if (typeof t3 !== "number")
      throw t3.$div();
    numRows = t3 / t4 + 1;
    this.points = P.List_List(C.JSNumber_methods.toInt$0(numRows), null);
    t4 = C.JSNumber_methods.toInt$0(numRows);
    fixedPoints = P.List_List(t4, null);
    y = size.top;
    x = size.left;
    row = 0;
    while (true) {
      ...
      if (!(y <= t3))
        break;
      t5 = this.points;
      t6 = P.List_List(C.JSNumber_methods.toInt$0(numColumns), null);
      if (row >= t5.length)
        throw H.ioore(t5, row);
      t5[row] = t6;
      t6 = P.List_List(C.JSNumber_methods.toInt$0(numColumns), null);
      if (row >= t4)
        throw H.ioore(fixedPoints, row);
      fixedPoints[row] = t6;
      x0 = x;
      column = 0;
      while (true) {
        ...
        if (column >= t5.length)
          throw H.ioore(t5, column);
        t5[column] = new K.PointMass(t6, new T.Vector3(t7), 1, new T.Vector3(t8), 0.98);
        t8 = fixedPoints[row];
        t7 = new Float32Array(3);
        t7.$dartCachedLength = t7.length;
        t7 = new T.Vector3(t7);
        ...
        if (column >= t8.length)
          throw H.ioore(t8, column);
        t8[column] = new K.PointMass(t7, new T.Vector3(t6), 0, new T.Vector3(t5), 0.98);
        ++column;
        x0 += t2[0];
      }
      ++row;
    ...
    for (t1 = numColumns - 1, t2 = numRows - 1, y = 0; y < numRows; ++y)
      for (t3 = y > 0, t5 = y !== 0, t6 = y === t2, t7 = y - 1, x = 0; x < numColumns; ++x) {
        if (x === 0 || y === 0 || x === t1 || t6) {
          t8 = this.springList;
          if (x >= t4)
            throw H.ioore(fixedPoints, x);
          t9 = fixedPoints[x];
          if (y >= t9.length)
            throw H.ioore(t9, y);
          t9 = t9[y];
          ...
          if (y >= t10.length)
            throw H.ioore(t10, y);
          t10 = new K.Spring(t9, t10[y], null, 0.1, 0.1);
          t9 = t10.End1.get$Position();
          t9 = t9.$sub(t9, t10.End2.get$Position());
          t10.TargetLength = t9.get$length(t9);
          t8.push(t10);
        } else if (C.JSInt_methods.$mod(x, 3) === 0 && C.JSInt_methods.$mod(y, 3) === 0) {
          t8 = this.springList;
          if (x >= t4)
            throw H.ioore(fixedPoints, x);
          t9 = fixedPoints[x];
          ...
          if (y >= t10.length)
            throw H.ioore(t10, y);
          t10 = new K.Spring(t9, t10[y], null, 0.002, 0.002);
          t9 = t10.End1.get$Position();
          t9 = t9.$sub(t9, t10.End2.get$Position());
          t10.TargetLength = t9.get$length(t9);
          t8.push(t10);
        ...
        if (x > 0) {
          ...
          if (y >= t9.length)
            throw H.ioore(t9, y);
          t9 = new K.Spring(t10, t9[y], null, 0.28, 0.6);
          t10 = t9.End1.get$Position();
          t10 = t10.$sub(t10, t9.End2.get$Position());
          t9.TargetLength = t10.get$length(t10);
          t8.push(t9);
        }
        if (t3) {
          t8 = this.springList;
          ...
          if (y >= t10)
            throw H.ioore(t9, y);
          t9 = new K.Spring(t11, t9[y], null, 0.28, 0.6);
          t11 = t9.End1.get$Position();
          t11 = t11.$sub(t11, t9.End2.get$Position());
          t9.TargetLength = t11.get$length(t11);
          t8.push(t9);
        }
      }
  },
  static: {
Grid$: function(size, spacing) {
  var t1 = new K.Grid(null, null);
  t1.Grid$2(size, spacing);
  return t1;
}}

}}],
["geometrywarsclone", "utils/PointMass.dart", , K, {
PointMass: {"": "Object;Position<,Velocity,InverseMass,acceleration,damping",
  ApplyForce$1: function(force) {
    var v, t1;
    v = force.$mul(force, this.InverseMass);
    t1 = this.acceleration;
    this.acceleration = t1.$add(t1, v);
  },
  Update$0: function() {
    var t1 = this.Velocity;
    this.Velocity = t1.$add(t1, this.acceleration);
    t1 = this.Position;
    this.Position = t1.$add(t1, this.Velocity);
    t1 = new Float32Array(3);
    t1.$dartCachedLength = t1.length;
    this.acceleration = new T.Vector3(t1);
    if (this.Velocity.get$length2() < 0.000001) {
      t1 = new Float32Array(3);
      t1.$dartCachedLength = t1.length;
      this.Velocity = new T.Vector3(t1);
    }
    t1 = this.Velocity;
    this.Velocity = t1.$mul(t1, this.damping);
    this.damping = 0.98;
  }
}}],
["geometrywarsclone", "utils/Spring.dart", , K, {
Spring: {"": "Object;End1,End2,TargetLength,Stiffness,Damping",
  Update$0: function() {
    var t1, t2, t3, x, $length, t4, force;
    t1 = this.End1;
    t2 = t1.get$Position();
    t3 = this.End2;
    x = t2.$sub(t2, t3.get$Position());
    $length = x.get$length(x);
    t2 = this.TargetLength;
    if (typeof t2 !== "number")
      throw H.iae(t2);
    if ($length <= t2)
      return;
    t2 = x.$div(x, $length);
    t4 = this.TargetLength;
    if (typeof t4 !== "number")
      throw H.iae(t4);
    x = t2.$mul(t2, $length - t4);
    t4 = t3.Velocity;
    force = x.$sub(x, t4.$sub(t4, t1.Velocity));
    force = force.$mul(force, this.Stiffness * this.Damping);
    t1.ApplyForce$1(force.$negate(force));
    t3.ApplyForce$1(force);
  }
}}],
["", "geometrywarsclone.dart", , G, {
main: function() {
  var t1, canvas, t2, t3;
  t1 = P.List_List(null, W.CanvasElement);
  H.setRuntimeTypeInfo(t1, [W.CanvasElement]);
  $.layers = t1;
  canvas = document.querySelector("#main_canvas");
  $.layers.push(canvas);
  $.ctx = J.get$context2D$x(canvas);
  t1 = new P.Rectangle(0, 0, 500, 500);
  ...
  t3[1] = 25;
  $.grid = K.Grid$(t1, t2);
  t2 = new W._ElementEventStreamImpl(canvas, C.EventStreamProvider_click._eventType, false);
  H.setRuntimeTypeInfo(t2, [null]);
  t1 = new W._EventStreamSubscription(0, t2._target, t2._eventType, W._wrapZone(G.addExplosion$closure), t2._useCapture);
  H.setRuntimeTypeInfo(t1, [H.getRuntimeTypeArgument(t2, "_EventStream", 0)]);
  t1._tryResume$0();
  G.setupCanvases($.layers);
  C.Window_methods.get$animationFrame(window).then$1(G.gameLoop$closure);
},
...
addExplosion: function(e) {
  ...
  t1.ApplyExplosiveForce$3(100, t2, 100);
},

setupCanvases: function(layers) {
  var t1, canvas, t2;
  for (t1 = new H.ListIterator(layers, layers.length, 0, null); t1.moveNext$0();) {
    canvas = t1._current;
    t2 = J.getInterceptor$x(canvas);
    t2.set$width(canvas, window.innerWidth);
    t2.set$height(canvas, window.innerHeight);
  }
},
...
init.globalFunctions.addExplosion$closure = G.addExplosion$closure = new P.Closure$1(G.addExplosion, "addExplosion$closure");
init.globalFunctions.gameLoop$closure = G.gameLoop$closure = new P.Closure$1(G.gameLoop, "gameLoop$closure");
...
function dart_precompiled($collectedClasses) {
  ...
  function Grid(points, springList) {
    this.points = points;
  ...
  if ($desc instanceof Array)
    $desc = $desc[1];
  Grid.prototype = $desc;
  function PointMass(Position, Velocity, InverseMass, acceleration, damping) {
  ...
  PointMass.builtin$cls = "PointMass";
  if (!"name" in PointMass)
    PointMass.name = "PointMass";
  $desc = $collectedClasses.PointMass;
  if ($desc instanceof Array)
    $desc = $desc[1];
  PointMass.prototype = $desc;
  PointMass.prototype.get$Position = function() {
    return this.Position;
  };
  function Spring(End1, End2, TargetLength, Stiffness, Damping) {
    this.End1 = End1;
    this.End2 = End2;
  ...
  Spring.builtin$cls = "Spring";
  if (!"name" in Spring)
    Spring.name = "Spring";
  $desc = $collectedClasses.Spring;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Spring.prototype = $desc;
